...
import { IncomeTypeSchema } from "./IncomeTypeSchema";
...
const IncomeTypeModal = ({ isOpen, onClose }: Props) => {
  ...
  const {
    ...
    resolver: yupResolver(IncomeTypeSchema),
    defaultValues: {},
  });

  const onSubmit = (values: any) => {
    addIncomeType.mutateAsync({ ...values, in_or_out: "in" }).then((res) => {
      if (res.error) {
        onClose();
        EToast({
          toast: toast,
